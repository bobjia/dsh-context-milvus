...
三组对比：**G**（grep 关键词）、**R**（朴素 RAG：固定滑窗 + 纯向量）、**P**（插件：AST 分块 + BM25 混合 + RRF 融合）。
...
- **AST 分块大幅降低检索噪音**：P vs R precision@10 +0.1263（p=0.000064，Cliff's Δ=0.905 大效应）。插件按函数/类边界分块，比固定滑窗的分块片段更聚焦，结果噪音显著降低。
- **语义检索排名效果优于 grep**：P vs G MRR +0.1579（p=0.083），nDCG@10 +0.1324（p=0.068）——相关文件排名更高，受限于样本量尚未达到统计显著。
