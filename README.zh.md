...
- **两阶段重排序提升 hit@1 达 6.7%**（相对 P0 基线从 0.714 提升至 0.762）：比例化术语重叠（+30%）和名称匹配（+15%）提升改善首项命中率，无激进多样性惩罚。与朴素 RAG（0.905）的差距从 0.19 缩小到 0.14。
- **precision@10 保持健康水平 0.273**（top-10 中约 2.7 个相关块）：reranker 采用比例化提升（相对 RRF 基准分），而非大幅绝对加分，Milvus 分数仍是主要排序信号。
