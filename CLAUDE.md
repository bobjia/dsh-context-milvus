...
This is a **DSH plugin** (DeepSeek's Cordis plugin framework) that provides semantic code search in a DSH agent environment. It indexes code into a **RemDB vector database** and searches it via natural language queries.
...
- `search_code` — semantic search against RemDB
...
index.ts (entry point)
  ...
  ├── remdb-service.ts — RemDB vector DB client wrapper (CRUD, search)
  ...
  └── indexer.ts    — indexing pipeline orchestration
        └── chunker.ts — tree-sitter AST chunking (TS/JS) + regex fallback (Python/Rust/Go/Java)
```
...
- **Incremental indexing**: `HashTracker` (merkle.ts) stores SHA-256 hashes per file in a local JSON file. On each index run, it compares current hashes against stored ones to produce a delta (toIndex / toRemove / unchanged). Tree-sitter AST parsing is only used for TypeScript/JavaScript; other languages (Python, Rust, Go, Java) use a regex-based chunker that detects function/class/method boundaries.
...
- **RemDB collection schema**: `{id, vector, file_path, code_content, start_line, end_line, language, chunk_type, name}` with COSINE metric on the vector index.
- **Embedding**: The `EmbeddingClient` calls an OpenAI-compatible API. Query text is embedded locally before vector search.
- **Batch operations**: RemDB inserts are batched at 100 rows; delete operations are done one file at a time.
...
Tests in `test/dsh-context-remdb.spec.ts` use ESM-compatible mocking (`jest.unstable_mockModule`). The `remdb-sdk-node` module is fully mocked; `EmbeddingClient` mocks override `globalThis.fetch`. Tests cover config resolution, HashTracker CRUD/persistence, embedding API calls, RemDB collection/search/insert/delete, and chunker output for all supported languages.